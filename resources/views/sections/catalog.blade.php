...
<div>
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
        <div>
            <span class="text-xs font-bold text-zinc-400 uppercase tracking-wider">{{ __('Gear Inventory') }}</span>
            <h3 class="text-2xl sm:text-3xl font-bold text-zinc-900 tracking-tight mt-1">{{ __('Product Catalog') }}</h3>
            <p class="text-zinc-500 text-sm mt-1">{{ __('Choose production gear tailored to your creative project needs.') }}</p>
        </div>
        ...
        <form action="{{ route('home') }}#catalog" method="GET" class="flex items-center gap-2" id="sortForm">
            ...
            <label for="urutan" class="text-xs font-medium text-zinc-500 whitespace-nowrap">{{ __('Sort by:') }}</label>
            <select 
                name="urutan" 
                id="urutan" 
            ...
            >
                <option value="">{{ __('Default (Featured)') }}</option>
                <option value="nama" {{ $currentSort == 'nama' ? 'selected' : '' }}>{{ __('Name (A - Z)') }}</option>
                <option value="termurah" {{ $currentSort == 'termurah' ? 'selected' : '' }}>{{ __('Lowest Price') }}</option>
                <option value="terbaru" {{ $currentSort == 'terbaru' ? 'selected' : '' }}>{{ __('Newly Added') }}</option>
            </select>
        </form>
    </div>
    ...
    @if($currentSearch)
        <div class="mb-6 flex items-center justify-between p-3.5 rounded-xl bg-zinc-50 border border-zinc-200/80 text-xs text-zinc-600">
            <span>{{ __('Search results for:') }} <strong class="text-zinc-900">"{{ $currentSearch }}"</strong></span>
            <a href="{{ route('home', array_filter(['kategori' => $currentKat, 'urutan' => $currentSort])) }}#catalog" class="text-zinc-500 hover:text-zinc-900 font-medium">{{ __('Clear keyword') }} &times;</a>
        </div>
    @endif
    ...
    @if($catalogs->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-zinc-200/80 border-dashed text-center px-4">
            ...
            <h4 class="text-base font-semibold text-zinc-900 mb-1">{{ __('Gear Not Found') }}</h4>
            <p class="text-zinc-500 text-xs max-w-sm mb-6">{{ __('Try picking another category or reset your search keyword.') }}</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="group bg-white rounded-2xl border border-zinc-200/80 overflow-hidden hover:border-zinc-300 hover:shadow-sm transition-all duration-200 flex flex-col">
                ...
                <div class="p-5 flex flex-col flex-grow justify-between">
                    <div>
                        <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{{ __($product->kategori) }}</span>
                    ...
                    <div class="mt-4 pt-3 border-t border-zinc-100">
                        <div class="flex items-baseline justify-between mb-3">
                            <span class="text-[11px] text-zinc-400">{{ __('Rental rate:') }}</span>
                            <p class="text-sm font-bold text-zinc-900">
                                Rp {{ number_format($product->harga_sewa, 0, ',', '.') }}<span class="text-[11px] font-normal text-zinc-400"> {{ __('/day') }}</span>
                            </p>
                        </div>
